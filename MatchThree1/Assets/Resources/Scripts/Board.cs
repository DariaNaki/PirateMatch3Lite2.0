...
public class Board : MonoBehaviour 
{
    ...
    void Start () 
	{
		...
		//gameObject.transform.position = new Vector3(-13f,-1f,0);
	}
	...
	public void CheckForMatches()
	{
		...
		BuildMatchList(gem1.color,gem1,gem1.XCoord,gem1.YCoord,ref gem1List);
		FixList(gem1,gem1List);
		BuildMatchList(gem2.color,gem2,gem2.XCoord,gem2.YCoord,ref gem2List);
		FixList(gem2,gem2List);
		if(!isMatched)
		{
			SwapBack = true;
			ResetGems();
	...
	public void CheckForNearbyMatches(Gem g)
	{
		...
		BuildMatchList(g.color,g,g.XCoord,g.YCoord,ref gemList);
		FixList(g,gemList);
	}
	
	public void BuildMatchList(string color,Gem gem,int XCoord,int YCoord, ref List<Gem> MatchList)
	{
		if(gem == null)
		{
			return;
		...
		else
		{
			...
			if(XCoord == gem.XCoord || YCoord == gem.YCoord)
			{
				...
				{
					BuildMatchList(color,g,XCoord,YCoord,ref MatchList);
				}
			}
		}
		...
		for(int i=0;i<gems.Count;i++)
		{
			for(int j =0;j<gems.Count;j++)
			{
				if(gems[i].IsNeighborWith(gems[j]))
				{
					...
					BuildMatchList(g.color,g,g.XCoord,g.YCoord,ref testListG);
					...
					BuildMatchList(f.color,f,f.XCoord,f.YCoord,ref testListF);
	...
	public void FixList(Gem gem, List<Gem> ListToFix)
	{
        ...
        if(gem.isStarted)
        {
            if (rows.Count == 3 || collumns.Count == 3)
                GameObject.Find("GameManager").GetComponent<GameManager>().Collect(100);
            if (rows.Count == 4 || collumns.Count == 4)
                GameObject.Find("GameManager").GetComponent<GameManager>().Collect(300);
            if(rows.Count >= 5)
            {
                GameObject.Find("GameManager").GetComponent<GameManager>().Collect(1000+(rows.Count-5)*500);
            }
            if (collumns.Count >= 5)
            {
                GameObject.Find("GameManager").GetComponent<GameManager>().Collect(1000 + (collumns.Count - 5) * 500);
            }
